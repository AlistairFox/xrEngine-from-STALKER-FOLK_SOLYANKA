...
// RENDER IMGUI 
void RenderSpawnManagerWindow(bool& IsActive)
{
...
bool SpawnManager_RenderButtonOrImage(CInifile::Sect* section, const char* imname)
{
	...
	float x = pSettings->r_float(name, "inv_grid_x") * INV_GRID_WIDTH;
	float y = pSettings->r_float(name, "inv_grid_y") * INV_GRID_HEIGHT;

	float w = pSettings->r_float(name, "inv_grid_width") * INV_GRID_WIDTH;
	float h = pSettings->r_float(name, "inv_grid_height") * INV_GRID_HEIGHT;

	ImGui::SeparatorText(name);

	return ImGui::ImageButton(imname,
...
void SpawnManager_HandleButtonPress(CInifile::Sect* section)
{
	...
	if (Actor())
	{
		...
		{
			const char* name = CStringTable().translate(pSettings->r_string(section->Name.c_str(), "inv_name")).c_str();
		...
		news_data.news_caption = CStringTable().translate("general_in_item");
	...
	// REWRITE TO OMP SPAWNER
	// string256 cmd;
	// sprintf(cmd, "%s %s %d", spawn_on_level || isInvItem ? "g_spawn " : "g_spawn_self_inv", section->Name.c_str(), count);
	// Msg("Execute: %s", cmd);
	// Console->Execute(cmd);
	...
	if (spawn_on_level)
	{
		...
		if (RQ.O)
		{
			Msg("! ОШИБКА: Возможно создать объект только на земле");
			return;
		}
	...
	Msg("Send Event Spawn Item: %s", section->Name.c_str());
...
void SpawnManager_RenderTooltip(CInifile::Sect* section)
{
	if (ImGui::BeginItemTooltip())
	{
		if (section->line_exist("inv_name"))
		{
			...
			{
				const auto& pTranslatedString = CStringTable().translate(pTranslateSectionName);
				ImGui::Text("In game name: [%s]", toUtf8(pTranslatedString.c_str()).c_str());
			}
		}
