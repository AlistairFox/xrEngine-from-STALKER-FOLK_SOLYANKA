...
void CMonsterEnemyMemory::update_mp()
{
	...
	// se7kills (Нахрен этот код пока не понятно почему падает преобразование константы)
	// TODO: Сделать что нибудь с этии :)
	/*
	if (monster->SoundMemory.IsRememberSound())
	{
		SoundElem sound;
	...
	*/
	...
	for (objects_list::const_iterator	I = objects.begin(); I != objects.end(); ++I)
	{
		...
		if (feel_enemy || monster->memory().visual().visible_now(*I))
		{
			...
			Msg("Radius Add Monster [%s, %d], Enemy [%s][%d], Distance [%f] ",
				monster->cName().c_str(), monster->ID(),
				enemy->cName().c_str(), enemy->ID(), 
