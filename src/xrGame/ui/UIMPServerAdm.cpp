...
CUIMpServerAdm::CUIMpServerAdm()
{
	...
	//Atmosfear 3 
	m_pWeatherAtmosfear3 = xr_new<CUI3tButton>();
	m_pWeatherAtmosfear3->SetAutoDelete(true);
	m_pMainSelectionWnd->AttachChild(m_pWeatherAtmosfear3);
	m_pWeatherAtmosfear3->SetMessageTarget(this);
	//End 
...
void CUIMpServerAdm::Init(CUIXml& xml_doc)
{
	...
	CUIXmlInit::Init3tButton(xml_doc, "atmosfear3:button", 0, m_pWeatherAtmosfear3);
	...
	case BUTTON_CLICKED:
		{
			if(pWnd==m_pBackBtn)
				OnBackBtn();
			else if (pWnd == m_pWeatherAtmosfear3)
			{
				ShowAtmosfear3();
			}
