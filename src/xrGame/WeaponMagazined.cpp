...
void CWeaponMagazined::OnEvent(NET_Packet& P, u16 type)
{
	...
	case GE_WPN_UPDATE_AMMO:
	{
		...
		//Msg("Set AmmoBox[%d] : AmmoSize: %d", BoxID, m_boxCurr);
...
void CWeaponMagazined::UnloadMagazine(bool spawn_ammo)
{
	...
	for (l_it = l_ammo.begin(); l_ammo.end() != l_it; ++l_it)
	{
		...
		if (l_it->second && !unlimited_ammo())
		{
			...
			{
				NET_Packet	P;
				CGameObject::u_EventGen(P, GE_WPN_UNLOAD_AMMO, ID());
				P.w_u16(l_it->second);
				P.w_stringZ(l_it->first);
...
void CWeaponMagazined::ReloadMagazine()
{
	...
	if (smart_cast<CActor*>(H_Parent()) && Level().CurrentControlEntity() != H_Parent())
		return;

	// —кипаем другие клиенты от перезар€дки (Ќам прелет€т патроны и так)
	if (OnClient() && !smart_cast<CActor*>(H_Parent()))
		return;
...
// 	Msg("Start Reloading ");
...

	{
		...
		if (m_pCurrentAmmo)
		{
			// Msg("Send AmmoSize: %d", m_pCurrentAmmo->m_boxCurr);
			NET_Packet					P;
...
void CWeaponMagazined::PlayAnimFireMode()
{
	...
	else if (strstr(guns_aim_anm_full, "_jammed"))
	{
		...
		xr_strcpy(new_guns_aim_anm, guns_aim_anm_full);
...
void CWeaponMagazined::PlayAnimLaserSwitch()
{
	...
	{
		char new_guns_aim_anm[256];
		xr_strcpy(new_guns_aim_anm, guns_device_switch_anm);
...
void CWeaponMagazined::PlayAnimAim()
{
	...
	if (const char* guns_aim_anm = GetAnimAimName())
	{
		...
		{
			char new_guns_aim_anm[256];
			xr_strcpy(new_guns_aim_anm, guns_aim_anm);
