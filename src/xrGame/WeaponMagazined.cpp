...
// se7kills !!!
// State Fire ќчень важный код 
// Ќужно убедитьс€ что не перестает стерл€ть очередь
...
void CWeaponMagazined::state_Fire(float dt)
{
	if (iAmmoElapsed > 0)
	{
		...
		{
			// se7kills (ѕеределать чтобы только в одном месте чекало на Misfire)
			// “еперь не будет блочитс€ очередь на сревере изза этого сервер не стрел€л
			if (CheckForMisfire())
			{
				StopShooting();
				return;
