...
void CWeapon::UpdatePosition(const Fmatrix& trans)
{
	...
	// ДЛЯ НАСТРОЙКИ ПОЛОЖЕНИЯ В САМОЙ ИГРЕ РАСКОМЕНТИТЬ
...
{	
	bool bIsActiveItem = m_pInventory && m_pInventory->ActiveItem()==this || H_Parent() && smart_cast<CActor*>(H_Parent());
...
//	Msg("SizeWeaponDEF[%d]", P.r_pos - pos);
	...
	NewAddonState = state.m_addon_flags.flags;

	ammoType = state.ammo_type;
	Zoom = state.m_bZoom;
