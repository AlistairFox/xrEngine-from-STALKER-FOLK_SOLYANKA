...
private:
	...
	shared_str				m_name_anim;

public:
	IC						CStalkerAnimationScript	(const MotionID &animation, shared_str name, bool hand_usage, bool use_movement_controller, Fmatrix const* transform = 0, bool local_animation = true);
	IC						CStalkerAnimationScript	(CStalkerAnimationScript const& object);
	IC	const MotionID&		animation				() const;
	IC	const LPCSTR		animation_name			() const;
