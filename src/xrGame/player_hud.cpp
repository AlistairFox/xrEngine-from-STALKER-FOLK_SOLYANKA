...
{ 
	...
	if (m_measures.m_prop_flags.test(hud_item_measures::e_fire_point))
	{
		...
		{
			Msg("m_measures.m_fire_bone %s", m_measures.m_fire_bone);
			return;
		}
	}
	...
	if (m_measures.m_prop_flags.test(hud_item_measures::e_fire_point2))
	{
		...
		{
			Msg("m_measures.m_fire_bone %s", m_measures.m_fire_bone2);
			return;
		}
	}
	...
	if (m_measures.m_prop_flags.test(hud_item_measures::e_shell_point))
	{
		...
		{
			Msg("m_measures.m_fire_bone: %s", m_measures.m_shell_bone);
			return;
		}
...
void hud_item_measures::load(const shared_str& sect_name, IKinematics* K)
{
	...
	strconcat(sizeof(val_name), val_name, "safemode_hud_offset_pos", _prefix);
	m_hands_offset[0][4] = READ_IF_EXISTS(pSettings, r_fvector3, sect_name, val_name, Fvector().set(0, 0, 0));
	//	Msg("Section: %s, %s: {%f, %f, %f}", sect_name.c_str(), val_name, VPUSH(m_hands_offset[0][4]));


	strconcat(sizeof(val_name), val_name, "safemode_hud_offset_rot", _prefix);
	m_hands_offset[1][4] = READ_IF_EXISTS(pSettings, r_fvector3, sect_name, val_name, Fvector().set(0, 0, 0));
	// 	Msg("Section: %s, %s: {%f, %f, %f}", sect_name.c_str(), val_name, VPUSH(m_hands_offset[1][4]));
...
player_hud::player_hud()
{
	...
	for (int i = 0; i < move_anms_end; i++)
	{
		...
		LPCSTR layer_def = pSettingsSe7kills->r_string("hud_movement_layers", temp);
...
const Fvector& player_hud::attach_pos(u8 part) const
{
	...
	// Msg("~~~ Strage Return Position For HUD[0,0,0] Actor[%.0f,%.0f,%.0f]", VPUSH(Actor()->Position()) );
	return Fvector().set(0.f, 0.f, 0.f);
}
