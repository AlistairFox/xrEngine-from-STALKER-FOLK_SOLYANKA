...
void CCustomDetector::ToggleDetector(bool bFastMode)
{
	...
	if (GetState() == eHidden)
	{
		...
		{
			Msg("¬озможность достать детектор: SlotTo: %u", slot_to_activate);
		...
		{
			Msg("Ќету возможности достать детектор. ");
		}
	}
	else
	{
		if (GetState() == eIdle)
		{
			Msg("ƒетектор уже достан, пр€чем. ");
			SwitchState(eHiding);
		}	
	}
}
...
{
	// Pavel: ѕредотвращение краша при дисконнекте сервера
	if (!Actor())
		return;
...
void CCustomDetector::OnMoveToRuck(const SInvItemPlace& prev)
{
	...
	if (prev.type == eItemPlaceSlot)
	{
		...
		// Pavel: фикс дл€ мп, не нужно доставать детектор,
		// если он был убран / заменЄн на другой во врем€ перезар€дки
		m_bNeedActivation = false;
	}
