...
animation_movement_controller::animation_movement_controller(Fmatrix* _pObjXForm, const Fmatrix& inital_pose, IKinematics* _pKinematicsC, CBlend* b) :
	m_startObjXForm(inital_pose), m_pObjXForm(*_pObjXForm),
	m_pKinematicsC(_pKinematicsC), m_pKinematicsA(smart_cast<IKinematicsAnimated*>(_pKinematicsC)),
	inital_position_blending(true), stopped(false),
	blend_linear_speed(0), blend_angular_speed(0),
	m_control_blend(b), m_poses_blending(Fidentity, Fidentity, -1.f)
{
	VERIFY(_pKinematicsC); VERIFY(m_pKinematicsA); VERIFY(_pObjXForm); VERIFY(b);
...
static bool get_animation_root_position(Fmatrix& pos, IKinematics* K, IKinematicsAnimated* KA, CBlend* control_blend)
{
	SKeyTable	keys;
...
bool animation_movement_controller::animation_root_position(Fmatrix& pos)
{
	return get_animation_root_position(pos, m_pKinematicsC, m_pKinematicsA, m_control_blend);
}
