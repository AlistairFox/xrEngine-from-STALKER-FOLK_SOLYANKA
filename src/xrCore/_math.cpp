...
void _initialize_cpu(void)
{
	...
	if (CPU::ID.hasFeature(CPUFeature::AMD_3DNow))
	...
	if (CPU::ID.hasFeature(CPUFeature::TM2))
		xr_strcat(features, ", TM2");

	if (CPU::ID.hasFeature(CPUFeature::AES))
		xr_strcat(features, ", AES");

	if (CPU::ID.hasFeature(CPUFeature::VMX))
		xr_strcat(features, ", VMX");

	if (CPU::ID.hasFeature(CPUFeature::EST))
		xr_strcat(features, ", EST");

	if (CPU::ID.hasFeature(CPUFeature::XFSR))
