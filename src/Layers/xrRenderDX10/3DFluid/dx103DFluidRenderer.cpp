...
/*
	...
	*/
	...
	RT[0] = nullptr;
	RT[0].create(m_pRTNames[0], width, height, RTFormats[0], SRV_RTV, 1);

	for (int i=1; i<RRT_NumRT; ++i)
	{
		RT[i] = 0;
		RT[i].create( m_pRTNames[i], m_iRenderTextureWidth, m_iRenderTextureHeight, RTFormats[i], SRV_RTV, 1);
	}
}
